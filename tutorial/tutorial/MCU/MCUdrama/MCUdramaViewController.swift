...
final class MCUdramaViewController: UIViewController {
    ...
    private let mcuDramaCell = R.nib.mcUdramaCell.name
...
extension MCUdramaViewController: CatchDataProtocol {
    func catchData(row: Int) {
        viewModel.getDramaDetailData(row: row)
        let vc = DetailViewController(with: viewModel)
        self.navigationController?.pushViewController(vc, animated: true)
    }
}

extension MCUdramaViewController: CatchFavoriteProtocol {
    func catchFavorite(row: Int) {
        viewModel.checkFavoriteStateDrama(row: row)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            ...
            if self.viewModel.isFavorite == [true] {
                ...
                alert.addAction(UIAlertAction(title: "削除", style: .default, handler: { [weak self] (_) in
                    self?.viewModel.checkSaveDrama(row: row)
...
extension MCUdramaViewController: StoryboardInstantiatable {
